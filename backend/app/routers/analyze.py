...
async def analyze_upload(file: UploadFile = File(...)):
    ...
    # The new PDF parser returns {"value": X, "ref_low": Y...}. We just need value.
    mapped_data = {k: v.get("value") for k, v in extracted_data.items() if isinstance(v, dict) and "value" in v}
    
    try:
        cbc_input = CBCInput(**mapped_data)
    except Exception as e:
